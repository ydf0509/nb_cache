这里为你提供了一版经过精心打磨、更具吸引力且能全面展现 `nb_cache` 核心竞争力的 `README` 开头介绍。

你可以直接复制以下 Markdown 内容替换掉原来的开头：

***

# nb_cache

🚀 **真正做到“同异步双栖”的工业级 Python 缓存与微服务治理中间件。**

`nb_cache` 不仅是一个基础的缓存装饰器，它更像是一把“瑞士军刀”，在彻底抹平同步与异步代码差异的同时，内置了双层缓存、防击穿、防雪崩、限流、熔断等企业级高可用特性。

## ✨ 核心特性

*   **☯️ 同异步无缝兼容**：无论是传统的同步函数 (`def`) 还是现代的异步协程 (`async def`)，统统只需**同一个装饰器**，内部自动路由，彻底告别 API 割裂。
*   **💾 多维存储后端**：支持 本地内存 (Memory)、Redis，以及开箱即用的 **L1 内存 + L2 Redis 双缓存架构 (Dual)**，大幅提升读写性能。
*   **🛡️ 极致的高并发保护**：
    *   **防击穿**：只需一句 `@cache(lock=True)` 即可开启互斥锁。
    *   **防穿透**：内置布隆过滤器 (`@bloom` / `@dual_bloom`)，从根源拦截无效查询。
    *   **防雷群**：自带并发调用去重 (`@thunder_protection`)，相同参数瞬间并发只执行一次真实计算。
*   **🌊 防雪崩的高级策略**：
    *   **失败回退 (`@failover`)**：下游服务报错时，自动掩盖异常并返回缓存的旧值。
    *   **提前刷新 (`@early` / `@soft`)**：到达临界时间点自动在**后台**触发异步刷新，用户请求不阻塞。
*   **🚦 微服务治理级组件**：除了缓存，还直接内置了 **限流器** (`@rate_limit` 固定/滑动窗口) 和 **熔断器** (`@circuit_breaker`)。
*   **🏷️ 现代化工程设计**：支持基于 **标签 (Tags)** 的业务维度批量缓存失效、支持对象属性与哈希的动态 Key 模板、支持安全签名与数据压缩，甚至支持缓存操作的**事务管理 (Transaction)**。

***

### 设计理念 (为什么要用 nb_cache？)
如果你的项目既有遗留的同步代码（如 Django、同步版 SQLAlchemy），又有高并发的异步代码（如 FastAPI），传统的纯异步缓存库（如 `cashews`）将让你痛苦不堪，而纯同步缓存库又无法发挥 Asyncio 的性能。**`nb_cache` 正是为这种混编架构和现代高可用系统而生。**
